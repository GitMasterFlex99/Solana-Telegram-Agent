...
describe("on-chain token safety", () => {
  it("detects active authorities and holder concentration", async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result: { value: { owner: "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA", data: { parsed: { info: { mintAuthority: "authority", freezeAuthority: "freeze", supply: "1000000", decimals: 0 } } } } } })))
      .mockResolvedValueOnce(new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result: { value: [{ amount: "250000", decimals: 0 }, { amount: "150000", decimals: 0 }, { amount: "100000", decimals: 0 }, { amount: "50000", decimals: 0 }, { amount: "50000", decimals: 0 }] } } }));
  ...
  it("identifies Token-2022", async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ result: { value: { owner: "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb", data: { parsed: { info: { supply: "100", decimals: 0 } } } } } })))
