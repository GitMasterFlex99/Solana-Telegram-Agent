...
describe("Pump.fun discovery source", () => {
  it("normalizes configured discovery results", async () => {
    const coins = await discoverPumpfun({
      ...
      fetchImpl: async () => response([{ mint: "abc", name: "Test", symbol: "TEST", created_at: 1_700_000_000_000, market_cap: 50_000 }]),
    });
    expect(coins).toHaveLength(1);
    expect(coins[0].address).toBe("abc");
  ...
  it("supports a coins envelope", async () => {
    const coins = await discoverPumpfun({
      endpoint: "https://example.invalid/pumpfun",
      fetchImpl: async () => response({ coins: [{ address: "abc", name: "Test", symbol: "TEST", createdAt: 1_700_000_000_000 }] }),
    });
    expect(coins).toHaveLength(1);
  });
