...
async function sendScan(ctx:any){const profile=await profileFor(ctx.from?.id);await ctx.reply(`🔎 Scanning with ${profile} risk settings...`);try{const pairs=await scan(profile);if(!pairs.length){await ctx.reply("No candidates passed the current filters.",{reply_markup:menu()});return;}for(const[pidx,p]of pairs.entries()){rememberPair(p);const candidate=classifyCandidate(p),address=p.baseToken?.address;const text=[`${pidx+1}. ${p.baseToken?.symbol??"Unknown"} — ${candidateLabel(candidate.classification)}`,`Opportunity ${p.opportunityScore}/100 · Momentum ${p.momentum.score}/100`,`Liquidity: ${money(p.liquidity?.usd)}   Volume: ${money(p.volume?.h24)}`,`24h: ${p.priceChange?.h24?.toFixed(1)??"—"}%   Age: ${age(p.pairCreatedAt)}`,`Why: ${candidate.reasons.slice(0,2).join(" · ")}`,"⚠️ Opportunity is not safety and is not a buy signal."].filter(Boolean).join("\n");await ctx.reply(text,{reply_markup:address?new InlineKeyboard().text("Analyze safety",`analyze:${address}`):menu()});}await ctx.reply("Analyze safety before considering any candidate. Trading is disabled.",{reply_markup:menu()});}catch{console.error("market scan failed");await ctx.reply("Couldn't fetch market data right now. Try again later.",{reply_markup:menu()});}}
...
bot.command("help",async ctx=>{if(guard(ctx))await ctx.reply("Use the buttons or /scan.\n\nThe bot never asks for a seed phrase or private key.\n\nWhen trading is enabled, use a separate trading wallet in Phantom or another mainstream Solana wallet. Do not connect your main wallet.",{reply_markup:menu()});});
...
bot.callbackQuery("home",async ctx=>{if(!guard(ctx)){await ctx.answerCallbackQuery({text:"Not authorized or rate limited."});return;}await ctx.answerCallbackQuery();await ctx.reply("Solana Meme Agent\n\nTrading is disabled for now.",{reply_markup:menu()});});
