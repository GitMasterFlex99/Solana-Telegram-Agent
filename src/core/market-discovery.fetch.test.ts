...
test("live discovery builds Solana pairs from token profiles", async () => {
  ...
  const fetchImpl = async (input: Parameters<typeof fetch>[0]) => {
    ...
    calls.push(url);
    if (url.includes("token-profiles/latest/v1")) return new Response(JSON.stringify([
      { chainId: "solana", tokenAddress: SOL }, { chainId: "solana", tokenAddress: TOKEN }, { chainId: "ethereum", tokenAddress: "0x123" }
    ]), { status: 200 });
    if (url.includes(`/tokens/v1/solana/`) && url.includes(TOKEN)) return new Response(JSON.stringify([
  ...
  assert.equal(calls.some(url => url.includes(SOL)), false);
});
