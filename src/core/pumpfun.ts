...
export function assessPumpfunSignal(coin: PumpfunCoin): PumpfunSignal {
  ...
  if (coin.stage === "bonding-curve") {
    score += 10;
    flags.push("Still on Pump.fun bonding curve");
  } else if (coin.stage === "pumpswap") {
    score += 5;
    flags.push("Graduated to PumpSwap");
  }

  if (ageHours <= 1) {
    score += 12;
    flags.push("Very early launch");
  } else if (ageHours <= 6) {
    score += 7;
    flags.push("Early launch");
  }

  if (coin.marketCapUsd > 0 && coin.marketCapUsd < 100_000) {
    score += 5;
    flags.push("Low market cap / high volatility zone");
  }

  if (coin.mayhemMode) {
    score -= 15;
    flags.push("Mayhem Mode enabled; automated trading can affect early price action");
  }
...
export function normalizePumpfunCoin(input: Record<string, unknown>): PumpfunCoin | null {
  ...
  if (!address || !name || !symbol || createdAt === null) return null;
  const marketCapUsd = typeof input.marketCapUsd === "number" ? input.marketCapUsd : typeof input.market_cap === "number" ? input.market_cap : 0;
  const completeUrl = typeof input.url === "string" ? input.url : `https://pump.fun/coin/${address}`;
  return {
    address, name, symbol, marketCapUsd, createdAt,
    description: typeof input.description === "string" ? input.description : undefined,
    imageUri: typeof input.imageUri === "string" ? input.imageUri : undefined,
