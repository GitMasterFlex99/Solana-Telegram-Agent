...
type AccountInfo = {
  value?: {
    owner?: string;
    data?: { parsed?: { info?: { mintAuthority?: string | null; freezeAuthority?: string | null; supply?: string; decimals?: number }; type?: string } };
  } | null;
};
type LargestAccounts = { value?: Array<{ amount?: string; uiAmount?: number | null; decimals?: number }> };

const TOKEN_PROGRAM = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA";
const TOKEN_2022_PROGRAM = "TokenzQdBNbLqP5VEhdkx3M4cT7Y5rJkR8H2pQw6L";
...
export async function inspectToken(rpcUrl: string, mint: string): Promise<OnchainSafety> {
  ...
  const amounts = accounts.map((a) => Number(a.amount ?? 0) / 10 ** (a.decimals ?? decimals)).filter(Number.isFinite);
